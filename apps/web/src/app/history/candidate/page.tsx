    ...
    useEffect(() => {
        async function loadHistory() {
            ...
            try {
                ...
                const res = await fetch(`${SERVER_URL}/api/history/candidate?${params}`, {
                    cache: "no-store",
                });
            ...
            <div className="mx-auto max-w-6xl">
                <header className="mb-6 flex flex-wrap items-end justify-between gap-4">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold">Candidate Sessions</h1>
                        <p className="mt-2 text-slate-300">Rooms joined by {actor?.username || "you"}.</p>
                    </div>
                ...
                {!loading && sessions.length === 0 ? (
                    <section className="rounded-xl border border-slate-800 bg-slate-900 p-6 text-slate-300">
                        No candidate history yet. Join a room to see it here.
                    </section>
                ) : null}
